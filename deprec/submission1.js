...
class Submission {
  ...
  async task(round) {
    try {
      ...
      // Play radio from the playlist
      await this.playRadioFromPlaylist('https://catchtherecord.com/files/1281691/reverie-field-1.m3u');
  ...
  /**
   * Plays radio audio from a playlist URL.
   *
   * @param {string} playlistURL - URL of the radio playlist
   */
  async playRadioFromPlaylist(playlistURL) {
    try {
      console.log('Fetching radio playlist:', playlistURL);
      const playlistResponse = await axios.get(playlistURL);
      const playlist = playlistResponse.data;

      // Extract audio URL from the playlist
      const audioURL = this.extractAudioURLFromPlaylist(playlist);

      if (audioURL) {
        // Play audio stream
        await this.playAudioFromURL(audioURL);
      } else {
        console.error('No audio URL found in the playlist');
      }
    } catch (error) {
      console.error('Error playing radio from playlist:', error);
    }
  }

  /**
   * Extracts the audio URL from the radio playlist.
   *
   * @param {string} playlist - Radio playlist content
   * @returns {string|null} - Audio URL or null if not found
   */
  extractAudioURLFromPlaylist(playlist) {
    // Implement logic to extract the audio stream URL from the playlist
    // For example, if the playlist is in M3U format, you might look for lines starting with "http" and return the first one
  ...
  async playAudioFromURL(audioURL) {
    ...
    // Create a new instance of Speaker
    const speaker = new Speaker({
      channels: 2,          // Number of audio channels (2 for stereo)
      bitDepth: 16,         // Number of bits per sample (16 for CD quality)
      sampleRate: 44100     // Sample rate (44100 Hz for CD quality)
    });
